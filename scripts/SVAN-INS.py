...
def annot_vntr(VCF, TRF_out, vntrBinDb):
    ...
    for variant in tqdm(VCF.variants):
        ...
        if vntr_bool:
            ...
            tmp = {'ITYPE_N': 'VNTR', 'PERC_RESOLVED': perc, 'NB_MOTIFS': nbMotifs, 'MOTIFS': motifs}
            variant.info.update(tmp)
...
def annot_tandem_dup(VCF, PAFs_ref):
    '''
    TO DO: IMPROVE CODE DOCUMENTATION/CLEANING
    '''
    ## Create VCF with DUP insertion calls
    dup_VCF = formats.VCF()
    dup_VCF.header = VCF.header

    dup_VCF.info_order = VCF.info_order
    dup_VCF.format_order = VCF.format_order
    
    ## Create VCF with non-DUP insertion
    no_dup_VCF = formats.VCF()
    no_dup_VCF.header = VCF.header

    no_dup_VCF.info_order = VCF.info_order
    ...
    for variant in tqdm(VCF.variants):
        ...
        if insId not in PAFs_ref:
            no_dup_VCF.add(variant)
            continue

        hits_PAF = copy.deepcopy(PAFs_ref[insId])
        ...
        for hit in hits_PAF.alignments:

            ## Not filter based on MAPQ as usually tandem duplication involve repetitive sequence
            #if hit.MAPQ == 0:
            #    continue
            ...
            hit_overlap = gRanges.overlap(hit.tBeg, hit.tEnd, iBeg, iEnd)[0]

            if hit_overlap:
                filteredHits.append(hit)

        hits_PAF.alignments = filteredHits
        ...
        ## Call duplication
        if chain.perc_query_covered() >= 50:
    
            #### Duplication type
            dupType = infer_duptype(chain)
            ...
            tmp = {'ITYPE_N': dupType, 'DUP_COORD': coords}
            variant.info.update(tmp)
    ...
    return dup_VCF, no_dup_VCF

def annot_tandem_dup_realignment(VCF, reference, rootDir):
    '''
    '''
    ## Create VCF with DUP insertion calls
    dup_VCF = formats.VCF()
    ...
    ## Load reference into a pysam object
    refIndex  = pysam.FastaFile(reference)
    ...
    for variant in tqdm(VCF.variants):
        ...
        if insId not in counter:
            counter[insId] = 1
        else:
            counter[insId] += 1         
        
        dirName = insId + '_' + str(counter[insId])
        outDir = rootDir + '/local_realign/' + dirName
        unix.mkdir(outDir)

        ## Align insert to breakpoint flanking region
        hits_flank = align2flankingRegion(variant, refIndex, 100, outDir)

        ## Filter hits based on mapping quality
        hits_flank.alignments = [hit for hit in hits_flank.alignments if hit.MAPQ >= 10]

        ## Discard if no hit passes the filters
        if not hits_flank.alignments:
            no_dup_VCF.add(variant)
            continue

        ## Create alignment chain
        chain = hits_flank.chain(10, 25)
        ...
        if chain.perc_query_covered() >= 75:
            ...
            ## Add duplication annotation to the variant info
            # NOTE: coordinates need to be translated from target region alignment to reference space
            coords = ','.join([hit.tName + ':' + str(hit.tBeg) + '-' + str(hit.tEnd) + '_' + hit.strand for hit in chain.alignments])
            tmp = {'ITYPE_N': dupType, 'DUP_COORD': coords}
            variant.info.update(tmp)
...
def infer_duptype(chain):
    '''
    '''
    #### Duplication type
    strands = [hit.strand for hit in chain.alignments]
...
def annot_tandem_dup_unmappable(VCF, PAFs_repeatDb):
    '''
    '''
    ## Create VCF with unmappable DUP insertion calls
    dup_unmap_VCF = formats.VCF()
    dup_unmap_VCF.header = VCF.header

    dup_unmap_VCF.info_order = VCF.info_order
    dup_unmap_VCF.format_order = VCF.format_order
    ...
    for variant in tqdm(VCF.variants):
        ...
        ## Discard if no hits on retrotransposon templates or not annotated repeat at the variant breakpoint position
        if (insId not in PAFs_repeatDb) or ('REPEAT_BKP' not in variant.info):
            no_dup_VCF.add(variant)
            continue
        ...
        templates = ','.join([i.split('|')[1] for i in templates])

        ## Collect info regarding the annotated repeat overlapping the breakpoint position
        repeatClass, family, interval, strand, millidiv = variant.info['REPEAT_BKP'].split(',')

        ## Call tandem duplication if:
        # - a) Inserted sequence aligns to a single repeat template AND
        # - b) Chain covers at least 75% of the query sequence AND
        # - c) Repeat template of the same family as the annotated repeat at the breakpoint position
        if (nbTemplates == 1) and (chain.perc_query_covered() >= 75) and (templates == family):
            
            #### Duplication type
            dupType = infer_duptype(chain)

            if (strand == '-') and (dupType == 'DUP'):
                dupType = 'INV_DUP'
            
            elif (strand == '-') and (dupType == 'INV_DUP'):
                dupType = 'DUP'

            ## Add duplication annotation to the variant info
            tmp = {'ITYPE_N': dupType, 'DUP_REPFAM': templates}
            variant.info.update(tmp)

            ## Add duplication call to the VCF
            dup_unmap_VCF.add(variant)

        else:
            no_dup_VCF.add(variant)

    return dup_unmap_VCF, no_dup_VCF

def annot_interspersed_dup(VCF, PAFs_ref):
    '''
    '''
    ## Create VCF with interspersed DUP insertion calls
    dup_interspersed_VCF = formats.VCF()
    dup_interspersed_VCF.header = VCF.header

    dup_interspersed_VCF.info_order = VCF.info_order
    dup_interspersed_VCF.format_order = VCF.format_order
    ...
    for variant in tqdm(VCF.variants):
        ...
        ## Insert does not align on the reference
        if (insId not in PAFs_ref) or ((insId in PAFs_ref) and (len(PAFs_ref[insId].alignments) == 0)):
            no_dup_VCF.add(variant)
            continue

        ## Chain reference hits reference
        chain = PAFs_ref[insId].chain(10, 25)
        ...
        if chain.perc_query_covered() >= 75:
            ...
            ## Add duplication call to the VCF
            dup_interspersed_VCF.add(variant)

        ## B) Not interspersed dup
        else:
            no_dup_VCF.add(variant)

    return dup_interspersed_VCF, no_dup_VCF

def annot_numt(VCF, PAFs_ref):
    '''
    '''
    ## Create VCF with NUMT insertion calls
    numt_VCF = formats.VCF()
    numt_VCF.header = VCF.header

    numt_VCF.info_order = VCF.info_order
    numt_VCF.format_order = VCF.format_order
    
    ## Create VCF without NUMT insertion
    NO_numt_VCF = formats.VCF()
    NO_numt_VCF.header = VCF.header

    NO_numt_VCF.info_order = VCF.info_order
    ...
    for variant in tqdm(VCF.variants):
        ...
        ## A) NUMT
        if (ref == 'chrM') and (chain.perc_query_covered() >= 75):

            ## Add duplication annotation to the variant info
            coords = ','.join([hit.tName + ':' + str(hit.tBeg) + '-' + str(hit.tEnd) for hit in chain.alignments])
            tmp = {'ITYPE_N': 'NUMT', 'MT_COORD': coords}
            variant.info.update(tmp)

            ## Add duplication call to the VCF
            numt_VCF.add(variant)
            
        ## B) Not NUMT
        else:
            NO_numt_VCF.add(variant)

    return numt_VCF, NO_numt_VCF
...
def annot_mei_not_retro(VCF, PAFs_repeatDb):
    ...
    for variant in tqdm(VCF.variants):
        ...
        if (chain.perc_query_covered() >= 75):
            ...
            tmp = {'ITYPE_N': 'solo_no_canonical', 'FAM_N': templates, 'PERC_RESOLVED': chain.perc_query_covered()}
            variant.info.update(tmp)
...
def call_retro_candidate(VCF):
    ...
    for variant in tqdm(VCF.variants):
        ...
        ## C) Poly(A) found
            ...
            percLen =  monomerA.length() / len(variant.alt) * 100
            noPolyLen = len(variant.alt) - monomerA.length()
            ...
            # b) RETRO candidate
                ...
                retro_annot[insId] = {}
                retro_annot[insId]['ITYPE_N'] = 'UNK'
                retro_annot[insId]['STRAND'] = '+'
                retro_annot[insId]['INS_LEN'] = len(variant.alt)
        ...
        ## D) Poly(T) found
            ...
            percLen =  monomerT.length() / len(variant.alt) * 100
            noPolyLen = len(variant.alt) - monomerT.length()
...
def align_3prime_td(retro_annot, exonsBinDb, reference, outDir):
    ...
    #PAF_path = '/Users/brodriguez/Research/Projects/1KGP_LR/SV_annot/resubmission/1KGP_callset/annot_INS/tmp/td3prime2ref.paf'
...
def annot_retroelement(retro_annot, exonsBinDb, repeatsDb, TRF, reference, outDir):
    ...
    retro_annot_pass = dict([(insId, retro_annot[insId]) for insId in retro_annot if (retro_annot[insId]['ITYPE_N'] != 'UNK')])
    retro_annot_fail = dict([(insId, retro_annot[insId]) for insId in retro_annot if (retro_annot[insId]['ITYPE_N'] == 'UNK')])
...
def filter_no_sva_hexamer(retro_annot):
    ...
    for insId in retro_annot:
        ...
        elif (retro_annot[insId]['ITYPE_N'] == 'PSD') or (retro_annot[insId]['ITYPE_N'] == 'orphan'):
            filter_pass[insId] = retro_annot[insId]
...
def filter_PSD_MAST2(retro_annot):
    ...
    for insId in retro_annot:
        ...
        if (retro_annot[insId]['ITYPE_N'] == 'PSD') and (retro_annot[insId]['SRC_GENE'] == 'MAST2'):
            filter_fail[insId] = retro_annot[insId]
        else:
            filter_pass[insId] = retro_annot[insId]
...
def filter_PSD_TEMPLATED(retro_annot):
    ...
    for insId in retro_annot:
        ...
        if (retro_annot[insId]['ITYPE_N'] == 'PSD') and (('5PRIME_TEMP_LEN' in retro_annot[insId]) or ('3PRIME_TEMP_LEN' in retro_annot[insId])):
            filter_fail[insId] = retro_annot[insId]
        else:
            filter_pass[insId] = retro_annot[insId]
...
def annotate_hexamer(VCF, retro_annot, TRF_out):
    ...
    for variant in tqdm(VCF.variants):
        ...
        if (insId not in retro_annot) or (insId not in TRF.callsDict):
            retro_annot[insId]['INS_TRIMMED'] = variant.alt
            continue
        
        hexamerSeq, retro_annot[insId]['INS_TRIMMED'] = TRF.callsDict[insId].SVA_hexamer(variant.alt, retro_annot[insId]['STRAND'], 10, 80)
...
def filter_perc_resolved(retro_annot):
    ...
    for insId in retro_annot:
        ...
        ## A) Orphan or PSD
        if (retro_annot[insId]['ITYPE_N'] == 'orphan') or (retro_annot[insId]['ITYPE_N'] == 'PSD'):
        ...
        ## B) Chimera
        elif (retro_annot[insId]['ITYPE_N'] == 'chimera'):
            filter_pass[insId] = retro_annot[insId]        

        ## C) solo or partnered without assigned family
        elif ((retro_annot[insId]['ITYPE_N'] == 'solo') or (retro_annot[insId]['ITYPE_N'] == 'partnered')) and ('FAM_N' not in retro_annot[insId]):

            filter_fail[insId] = retro_annot[insId]                

        ## D) solo or partnered SVA
        elif ((retro_annot[insId]['ITYPE_N'] == 'solo') or (retro_annot[insId]['ITYPE_N'] == 'partnered')) and (retro_annot[insId]['FAM_N'] == 'SVA'):
        ...
        elif ((retro_annot[insId]['ITYPE_N'] == 'solo') or (retro_annot[insId]['ITYPE_N'] == 'partnered')):
...
def filter_partnered_ipos(retro_annot):
    ...
    for insId in retro_annot:
        ...
        if retro_annot[insId]['ITYPE_N'] != 'partnered':
            filter_pass[insId] = retro_annot[insId]
            continue
...
def reclassify_5prime_partnered_ipos(retro_annot):
    ...
    for insId in retro_annot:
        ...
        if (retro_annot[insId]['ITYPE_N'] != 'partnered') or ('5PRIME_TD_COORD' not in retro_annot[insId]):
            continue
...
def transductions_5prime_refinement(retro_annot, reference, exonsBinDb, minMAPQ, minHitLen, outDir):
    ...
    for insId in retro_annot:
        ...
        if ('5PRIME_TD_LEN' in retro_annot[insId]) or (retro_annot[insId]['ITYPE_N'] == 'orphan') or (retro_annot[insId]['ITYPE_N'] == 'PSD'):
            continue
    ...
    #PAF_path = '/Users/brodriguez/Research/Projects/1KGP_LR/SV_annot/resubmission/1KGP_callset/annot_INS/tmp/5prime_td_candidates.paf'
    ...
    for insId in PAFs_hits:
        ...
        if chain.perc_query_covered() > 80:
            ...
            retro_annot[insId]['ITYPE_N'] = 'partnered'
...
def compute_perc_resolved(retro_annot):
    ...
    if (retro_annot['ITYPE_N'] == 'UNK'):
        retro_annot['PERC_RESOLVED'] = 0

    ## b) Solo insertion type
    elif (retro_annot['ITYPE_N'] == 'solo'):
        ...
        retro_annot['PERC_RESOLVED'] = resolvedLen / retro_annot['INS_LEN'] * 100

    ## c) Partnered transduction
    elif (retro_annot['ITYPE_N'] == 'partnered'):
    ...
    elif (retro_annot['ITYPE_N'] == 'orphan'):
        ## General
        tsd_len = int(retro_annot['TSD_LEN']) if 'TSD_LEN' in retro_annot else 0
...
def solo_annot(retro_annot, chain):
    ...
    if (retro_annot['ITYPE_N'] == 'UNK') and ('FAM_N' in retro_annot.keys()):
        retro_annot['ITYPE_N'] = 'solo'
...
def transductions_5prime(retro_annot, chain_consensus, PAF_ref, exonsBinDb):
    ...
    if (retro_annot['ITYPE_N'] == 'PSD') or (retro_annot['ITYPE_N'] == 'orphan') or ('5PRIME_TEMP_SEQ' in retro_annot) or (len(PAF_ref.alignments) == 0) or ((nbTemplates == 1) and ('Alu' in templates)) or (unalignedLen < minTdLen):
        return retro_annot
...
def transductions_3prime_hits(retro_annot, chain_consensus, PAF_ref, exonsBinDb):
    ...
    if (retro_annot['ITYPE_N'] == 'PSD') or (retro_annot['ITYPE_N'] == 'orphan') or (retro_annot['ITYPE_N'] == 'partnered') or ('5PRIME_TEMP_SEQ' in retro_annot) or (len(PAF_ref.alignments) == 0) or ((nbTemplates == 1) and ('Alu' in templates)):
        return retro_annot
...
def orphan_td_annot(retro_annot, chain):
    ...
    if ('consensus' not in templates) and (retro_annot['ITYPE_N'] == 'UNK') and (nbTemplates == 1) and (chain.perc_query_covered() >= 75):
        retro_annot['ITYPE_N'] = 'orphan'
...
def alignTrimmedIns2ref(retro_annot, reference, minMAPQ, minHitLen, fileName, outDir):
    ...
    preset = 'sr'
    #PAF_short_path = '/Users/brodriguez/Research/Projects/1KGP_LR/SV_annot/resubmission/1KGP_callset/annot_INS/tmp/trimmed2ref_short.paf'
    ...
    preset = 'map-ont'
    #PAF_long_path = '/Users/brodriguez/Research/Projects/1KGP_LR/SV_annot/resubmission/1KGP_callset/annot_INS/tmp/trimmed2ref_long.paf'
...
def alignTrimmedIns2repeats(retro_annot, reference, minMAPQ, minHitLen, fileName, outDir):
    ...
    #PAF_path = '/Users/brodriguez/Research/Projects/1KGP_LR/SV_annot/resubmission/1KGP_callset/annot_INS/tmp/trimmed2repeats.paf'
...
def nested_annot(retro_annot, reference, minMAPQ, minHitLen, fileName, outDir):
    ...
    for insId in consHitsTd:
        ...
        if (chain.perc_query_covered() > 80) and (last_op != 'TRUN'):
            ...
            ## Create nested entry
            retro_annot[insId]['ITYPE_N'] = 'solo'
...
def family_annot_insert(retro_annot, chain):
    ...
    if (retro_annot['ITYPE_N'] == 'PSD') or (retro_annot['ITYPE_N'] == 'orphan'):
        return retro_annot
...
def align_ins2ref(VCF, reference, outDir):
    ...
    #PAF_short_path = '/Users/brodriguez/Research/Projects/1KGP_LR/SV_annot/resubmission/1KGP_callset/annot_INS/tmp/inserts2ref_short.paf'
    preset = 'sr'
    ...
    #PAF_long_path = '/Users/brodriguez/Research/Projects/1KGP_LR/SV_annot/resubmission/1KGP_callset/annot_INS/tmp/inserts2ref_long.paf'
    preset = 'map-ont'
...
def align_ins2repeats(VCF, repeatsDb, outDir):
    '''
    '''
    ## Write fasta with inserted sequence
    FASTA_path = outDir + '/inserted_sequences.fa'
    fasta = ins2fasta(VCF)
    fasta.write(FASTA_path)

    ## Align inserted sequence against reference sequence
    #PAF_path = '/Users/brodriguez/Research/Projects/1KGP_LR/SV_annot/resubmission/annot_INS/tmp/inserts2repeats.paf'
    SAM_path = alignment.alignment_bwa(FASTA_path, repeatsDb, 'inserts2repeats', 2, outDir)     
    PAF_path = alignment.sam2paf(SAM_path, 'inserts2repeats', outDir)
    
    PAF_repeats = formats.PAF()
    PAF_repeats.read(PAF_path)

    ## Group alignments by insertion
    repeatsHitsIns = group_alignments(PAF_repeats)
 
    return repeatsHitsIns

def annotate_repeats_bkp(VCF, repeatsBinDb):
    '''
    '''
    ...
    for variant in tqdm(VCF.variants):
        ...
        if overlaps: 
            annot = overlaps[0][0]
            annotInfo = annot.optional['class'] + ',' + annot.optional['family'] + ',' + annot.optional['repBeg'] + '-' + annot.optional['repEnd'] + ',' + annot.optional['strand'] + ',' + annot.optional['milliDiv']

            ## Update info field with repeat annotation details
            tmp = {'REPEAT_BKP': annotInfo}
            variant.info.update(tmp)
...
def annot_retro_lenient(VCF, reference, repeatsDb, tmpDir):
    '''
    '''
    ## 1. Annotate and trim TSD  
    ############################
    ...
    for variant in tqdm(VCF.variants):
        ...
        annot[insId]['ITYPE_N'] = 'UNK'
        annot[insId]['FAM_N'] = 'UNK'
    ...
    ## 2. Annotate polyA tails
    ###############################
    # == INS with a poly(A/T) at one of its ends
    polyA_VCF, noPolyA_VCF, polyA_annot, stats = call_retro_candidate(VCF)

    # Add polyA info to the annotation dictionary
    for insId in polyA_annot:
        annot[insId]['STRAND'] = polyA_annot[insId]['STRAND']
        annot[insId]['POLY_MONOMER'] = polyA_annot[insId]['POLY_MONOMER']

    ## Report some stats 
    for i in ['POLYA?', 'HOMO_A', 'HOMO_T', 'NO_TAIL', 'DOUBLE_TAIL']:
        if i not in stats:
            continue
        print('NB_' + i, ': ', stats[i])

    ## Strand for those with polyAs
    counts = pd.Series([polyA_annot[i]['STRAND'] for i in polyA_annot.keys()]).value_counts()
    print('STRAND_CANDIDATES: ', counts)
    
    ## 3. Annotate and trim PolyA
    ###############################
    annotate_trim_polyA(annot)

    ## 4. Align into the repeats database
    ######################################

    ## Align trimmed inserts against retroelements database 
    PAFs_repeats = alignTrimmedIns2repeats2(annot, repeatsDb, 0, 15, 'trimmed2repeats_lenient', tmpDir)

    ## Create hit chains, one per insertion
    for insId in PAFs_repeats.keys():
        annot[insId]['CHAIN_repeats'] = PAFs_repeats[insId].chain(300, 25) ## NOTE: parameters to tune, specially the distance threshold

    ## 5. Family assignation
    #########################
    for insId in PAFs_repeats.keys():

        annot[insId] = family_annot_insert(annot[insId], annot[insId]['CHAIN_repeats'])

    ## 6. Solo retrotransposon annotation
    ######################################
    for insId in PAFs_repeats.keys():
        annot[insId] = solo_annot(annot[insId], annot[insId]['CHAIN_repeats'])

    ## 7. Filtering and conformation annotation for retroelement calls
    ###################################################################
    ## Initialize retroelement VCF
    RETRO_VCF = formats.VCF()
    RETRO_VCF.header = VCF.header
    RETRO_VCF.info_order = VCF.info_order
    RETRO_VCF.format_order = VCF.format_order

    ## Initialize non-retroelement VCF
    NO_RETRO_VCF = formats.VCF()
    NO_RETRO_VCF.header = VCF.header
    ...
    for variant in tqdm(VCF.variants):
        ...
        ## Initialize insertion status as not canonical
        annot[insId]['NOT_CANONICAL'] = True

        ## Solo Alu or L1 retroelement with enough % resolved
        if (annot[insId]['ITYPE_N'] == 'solo') and (annot[insId]['FAM_N'] in ['L1', 'Alu']) and (annot[insId]['CHAIN_repeats'].perc_query_covered() >= 75):

            ## Annotate structure
            annot[insId] = annotate_structure(annot[insId])
            
            ## Determine if canonical insertion conformation
            if ('CONFORMATION' in annot[insId]):

                ## L1
                if (annot[insId]['FAM_N'] == 'L1'):
                    canonicalList = ['FOR+POLYA', 'TRUN+FOR+POLYA', 'TRUN+REV+DEL+FOR+POLYA', 'TRUN+REV+DUP+FOR+POLYA', 'TRUN+REV+BLUNT+FOR+POLYA', 'FOR+POLYA+TD+POLYA', 'TRUN+FOR+POLYA+TD+POLYA', 'TRUN+REV+DEL+FOR+POLYA+TD+POLYA', 'TRUN+REV+DUP+FOR+POLYA+TD+POLYA', 'TRUN+REV+BLUNT+FOR+POLYA+TD+POLYA', 'TD+FOR+POLYA', 'REV+DEL+FOR+POLYA']
                ...
                CANONICAL = is_canonical(annot[insId]['CONFORMATION'], canonicalList)

                if CANONICAL:
                    annot[insId]['NOT_CANONICAL'] = False

            ## Add percentage resolved
            annot[insId]['PERC_RESOLVED'] = annot[insId]['CHAIN_repeats'].perc_query_covered()

            ## Add annotation to the VCF info field
            variant.info.update(annot[insId])

            ## Add RETRO to the VCF
            RETRO_VCF.add(variant)

        ## SVA with enough % resolved
        elif (annot[insId]['ITYPE_N'] == 'solo') and (annot[insId]['FAM_N'] == 'SVA') and (annot[insId]['CHAIN_repeats'].perc_query_covered() >= 75):
        
            ## Structure
            annot[insId] = annotate_structure_sva(annot[insId])
            ...
            ## VNTR
            annot[insId] = annotate_VNTR(annot[insId])
        ...
        ## False positive HERVK
        elif ('FAM_N' in annot[insId]) and (annot[insId]['FAM_N'] == 'HERVK'):

            ## Add RETRO to the VCF
            NO_RETRO_VCF.add(variant)

        ## Other insertion class (i.e. ERVK/LTR) 
        elif ((annot[insId]['ITYPE_N'] == 'solo') or (annot[insId]['ITYPE_N'] == 'chimera')) and (annot[insId]['CHAIN_repeats'].perc_query_covered() >= 75):

            ## Remove TSD annotation
            if 'TSD_LEN' in annot[insId]:
                del annot[insId]['TSD_LEN']
                del annot[insId]['TSD_SEQ']

            ## Remove RT length
            if 'RT_LEN' in annot[insId]:
                del annot[insId]['RT_LEN']
...
def annotate_trim_polyA(annot):
    '''
    '''
    for insId in annot:

        if 'STRAND' not in annot[insId]:
            continue

        ## A) Insertion in forward
        if annot[insId]['STRAND'] == '+':
            
            insert = annot[insId]['INS_TRIMMED']
            annot[insId]['POLYA_LEN'] = str(annot[insId]['POLY_MONOMER'].length())
            annot[insId]['POLYA_SEQ'] = annot[insId]['INS_TRIMMED'][annot[insId]['POLY_MONOMER'].beg:annot[insId]['POLY_MONOMER'].end]
            annot[insId]['INS_TRIMMED'] = annot[insId]['INS_TRIMMED'][:annot[insId]['POLY_MONOMER'].beg]

        ## B) Insertion in reverse
        else:
            insert = annot[insId]['INS_TRIMMED']
            annot[insId]['POLYA_LEN'] = str(annot[insId]['POLY_MONOMER'].length())
            annot[insId]['POLYA_SEQ'] = annot[insId]['INS_TRIMMED'][annot[insId]['POLY_MONOMER'].beg:annot[insId]['POLY_MONOMER'].end]
            annot[insId]['INS_TRIMMED'] = annot[insId]['INS_TRIMMED'][annot[insId]['POLY_MONOMER'].end:]
...
## 0.B Add insertion legnth to each variant
###############################
### Read VCF (6" with no genotypes)
VCF = formats.VCF()
VCF.read(vcf)

### Update variant info field by adding INS_LEN 
for variant in VCF.variants:
    annot = {}
    annot['INS_LEN'] = len(variant.alt) 
    variant.info.update(annot)

## 0.C Filter out INS longer than a threshold
##############################################
### VCF filtering
maxLen = 50000
...
print('1.b VNTR database')
#vntrBinDb = formats.bed2binDb(vntrDb, filtered_VCF.header.refLengths, 2)
vntrBinDb = None

### Create bin database containing exons (30")
print('1.c Annotated exons')
exonsBinDb = formats.bed2binDb(exons, filtered_VCF.header.refLengths, 2)

### Load annotated repeats
print('1.d Annotated repeats')
#repeatsBinDb = formats.bed2binDb(repeatsAnnot, filtered_VCF.header.refLengths, 2)

## 2. Annotate repetitive sequences at the insertion breakpoints
#################################################################
...
## 4. Annotate retrotransposed insertions
##########################################
print('4. Annotate retrotransposed insertions')
...
## 5. Annotate VNTR events
###########################
print('5. Annotate VNTR events')
vntr_VCF, unk_VCF = annot_vntr(unk_VCF, TRF_out, vntrBinDb)
print('NB_VNTR: ', len(vntr_VCF.variants))
print('NB_UNK: ', len(unk_VCF.variants))

## 6. Annotate mappable tandem duplication events
###################################################
print('6. Annotate mappable tandem duplication events')
dup_VCF, unk_VCF = annot_tandem_dup(unk_VCF, PAFs_ref)
dupTypes = [variant.info['ITYPE_N'] for variant in dup_VCF.variants]

print('NB_DUP: ', dupTypes.count('DUP'))
print('NB_INV_DUP: ', dupTypes.count('INV_DUP'))
print('NB_COMPLEX_DUP: ', dupTypes.count('COMPLEX_DUP'))
print('NB_UNK: ', len(unk_VCF.variants))
...
######################################################
#print('8.a Align inserts to repeats database')
#PAFs_repeatDb = align_ins2repeats(unk_VCF, repeatsDb, tmpDir)

#print('8.b Annotate not mappable tandem duplication events')
#dup_unmap_VCF, unk_VCF = annot_tandem_dup_unmappable(unk_VCF, PAFs_repeatDb)
#dupTypes = [variant.info['ITYPE_N'] for variant in dup_unmap_VCF.variants]

#print('NB_DUP_UNMAP: ', dupTypes.count('DUP'))
#print('NB_INV_DUP_UNMAP: ', dupTypes.count('INV_DUP'))
#print('NB_COMPLEX_DUP_UNMAP: ', dupTypes.count('COMPLEX_DUP'))
#print('NB_UNK: ', len(unk_VCF.variants))

## 9. Call NUMT
#################
print('9. Call NUMT')
numt_VCF, unk_VCF = annot_numt(unk_VCF, PAFs_ref)

print('NB_NUMT: ', len(numt_VCF.variants))
print('NB_UNK: ', len(unk_VCF.variants))

## 10. Annotate MEI without retrotransposition hallmarks
########################################################
print('10. Annotate MEI without retrotransposition hallmarks')
retro2_VCF, unk_VCF = annot_retro_lenient(unk_VCF, reference, repeatsDb, tmpDir)

#mei_not_retro_VCF, unk_VCF = annot_mei_not_retro(unk_VCF, PAFs_repeatDb)
print('retro2_VCF: ', len(retro2_VCF.variants))
print('NB_UNK: ', len(unk_VCF.variants))


## 11. Annotate interspersed duplication
##########################################
print('11. Annotate interspersed duplication')
dup_interspersed_VCF, unk_VCF = annot_interspersed_dup(unk_VCF, PAFs_ref) 

print('dup_interspersed_VCF: ', len(dup_interspersed_VCF.variants))
print('NB_UNK: ', len(unk_VCF.variants))
...
#for variant in unk_VCF.variants:
    ...
    #insId = variant.ID + '_' + variant.chrom + ':' + str(variant.pos) 
    #print('REMAINING_UNK: ', insId, len(variant.alt), variant.alt)

    ## Insert does not align on the reference
    #if (insId not in PAFs_ref) or ((insId in PAFs_ref) and (len(PAFs_ref[insId].alignments) == 0)):
    #print('INSERT_WITHOUT_HITS: ', insId, len(variant.alt), variant.alt)

    ## Insert aligns on the reference
    #else:
    #    chain = PAFs_ref[insId].chain(20, 50)
    #    print('INSERT_WITH_REF_HITS: ', insId, chain.perc_query_covered(), len(variant.alt), variant.alt)

## 13. Write output VCF files with annotations
##############################################
print('13. Write output VCF files with annotations')
...
info2add = {'ITYPE_N': ['.', 'String', 'Type of insertion: solo, partnered, orphan, PSD (Processed pseudogene), VNTR'], \
            'INS_LEN': ['1', 'Integer', 'Insertion length'], \
